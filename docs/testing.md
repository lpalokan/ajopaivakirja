...
| `Then the sensor diagnostics show {string}` / `do not show {string}` | Asserts what Settings → Vianmääritys → Anturien käyttö is naming as a current holder. Android's battery screen says how long the GPS was on and never who asked, and the cheap idle watch delivers nothing from a parked car — so "held but silent" and "not held" are indistinguishable without this |
| `Then the sensor diagnostics show nothing is held` | Asserts the app holds no platform subscription at all. Hours of GPS in a battery report is this assertion failing on a real phone |
| `Given cancelling the driving notification fails` | Makes one step of the trip teardown throw, the way a platform channel can. It sits between the trip ending and the GPS being released, which is where an unguarded chain used to lose the location foreground service |
| `Given stopping the trip location stream fails once` | Makes the next attempt to close the trip's position stream throw with the stream still open — the one failure the teardown's own guards cannot recover from, since it is the teardown. The attempt after it succeeds |
| `When the sensor watchdog runs` | Shortens the watchdog interval (test seam) and pumps past several ticks, so the check that a trip's location hold still belongs to a trip fires. It acts only on the second consecutive ownerless tick, so an ordinary teardown in flight is not read as a leak |
